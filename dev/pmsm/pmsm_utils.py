...
def PMSM_penalty(env, observations, actions, penalty_order=2):
    ...
    obs_penalty = jax.nn.relu(a**2 + b**2 - 0.9) ** penalty_order
...
def plot_constraints_induced_voltage(env, physical_i_d, physical_i_q, w_el, saturated=True):
    ...
    umult = 2 / 3
    plt.contour(Id, Iq, I, levels=[250, 250 + 1e-6], colors="r", linewidths=2)
    # plt.contour(Id, Iq, T, linestyles='dashed', colors='k')
    plt.contour(Id, Iq, U, levels=[udc * umult, (1 + 1e-6) * udc * umult], colors="b", linewidths=2)
